...
def parse_args():
    ...
    parser.add_argument(
        '--population-size', help='Population size.', type=int, default=128)
    parser.add_argument(
        '--num-workers', help='Number of workers.', type=int, default=-1)
    parser.add_argument(
        '--num-gpus', help='Number of GPUs for training.', type=int, default=-1)
    parser.add_argument(
        '--max-iter', help='Max training iterations.', type=int, default=1000)
    parser.add_argument(
...
def worker_init(config_file, device_type, num_devices):
    ...
    print("Worker Device:", device) # Addition
...
def main(config):
    ...
    solution = util.create_solution(device='cpu:0' if args.num_gpus <= 0 else 'cuda:0')
